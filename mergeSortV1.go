...
func dividing( arr []int) []int{
	if len(arr) > 1{
		
		
		leftHold := func() chan []int{
			lCh :=make(chan []int)
			go func(){
				lCh <- arr[0:len(arr)/2]
				return
		...
		leftArr = dividing(leftArr)
		rightArr = dividing(rightArr)

		
		mergeNsortedArr := mergeSort(leftArr,rightArr)
